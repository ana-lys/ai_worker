...
FILE *open_ffmpeg_sender(const std::string &ip, int port, int width, int height,
  ...
  if (codec == "h264") {
    ...
    cmd << "-c:v mjpeg -q:v 3 -pix_fmt yuvj420p ";
  }
...
void stream_camera(const std::string &serial, int index,
  ...
  hdr << "\n=== CAM" << index << " " << serial << " : depth->udp:" << depth_port
      << "  ir->udp:" << ir_port << "  codec=" << codec << " ===";
  log(hdr.str());

  FILE *depth_pipe =
      open_ffmpeg_sender(dest_ip, depth_port, width, height, fps, codec);
  FILE *ir_pipe =
      open_ffmpeg_sender(dest_ip, ir_port, width, height, fps, codec);
...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0]
              << " <dest_ip> <base_port> [width=480] [height=270] [fps=30] "
                 "[max_depth_m=1.0] [codec=mjpeg|h264]"
              << std::endl;
    return 1;
  }
  ...
  std::string codec = argc > 7 ? argv[7] : "mjpeg";

  if (codec != "mjpeg" && codec != "h264") {
    std::cerr << "Invalid codec '" << codec << "' -- must be 'mjpeg' or 'h264'"
              << std::endl;
    return 1;
  }
