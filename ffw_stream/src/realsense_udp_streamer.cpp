...
public:
    RealsenseUDPStreamer() : Node("realsense_udp_streamer") {
        ...
        if (this->get_parameter("enable_rgb").as_bool()) {
            ...
            std::string pipe_str = "appsrc name=src is-live=true do-timestamp=true format=time ! video/x-raw,format=RGB,width=" + std::to_string(w) + 
        ...
        if (this->get_parameter("enable_depth").as_bool()) {
            ...
            std::string pipe_str = "appsrc name=src is-live=true do-timestamp=true format=time ! video/x-raw,format=UYVY,width=" + std::to_string(w) + 
                                   ",height=" + std::to_string(h) + ",framerate=" + std::to_string(fps) + "/1 ! "
                                   "rtpvrawpay ! udpsink host=" + target_ip_ + " port=" + std::to_string(port) + " max-bitrate=120000000";
        ...
        if (this->get_parameter("enable_ir").as_bool()) {
            ...
            std::string pipe_str = "appsrc name=src is-live=true do-timestamp=true format=time ! video/x-raw,format=GRAY16_LE,width=" + std::to_string(w) + 
                                   ",height=" + std::to_string(h) + ",framerate=" + std::to_string(fps) + "/1 ! "
