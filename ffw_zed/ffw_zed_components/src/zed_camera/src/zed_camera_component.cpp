...
void ZedCamera::getGeneralParams()
{
  ...
  if (mSvoFilepath == "") {
    ...
    if (!mSvoRealtime) {
      getParam("svo.replay_rate", mSvoRate, mSvoRate, " * SVO replay rate: ", true);
      if (mSvoRate < 0.1 || mSvoRate > 5.0) {
        RCLCPP_WARN_STREAM(
          get_logger(),
          "The parameter 'svo.replay_rate' is not valid, using the default value: "
            << mSvoRate);
        mSvoRate = 1.0;
      }
    }
...
void ZedCamera::threadFunc_zedGrab()
{
  ...
  std::thread ffw_pipe_thread([&]() {
    RCLCPP_INFO(get_logger(), "FFMPEG Pipe thread started.");
    size_t expected_size = ffw_width * ffw_height * 4;
    while (ffw_running) {
      int slot = -1;
      {
          std::unique_lock<std::mutex> lock(ffw_mtx);
          ffw_cv.wait(lock, [&] { return !ffw_q.empty() || !ffw_running; });
          if (!ffw_running && ffw_q.empty()) break;
          slot = ffw_q.front();
          ffw_q.pop();
      }
      ...
      fflush(ffw_stream_pipe);
    }
    RCLCPP_INFO(get_logger(), "FFMPEG Pipe thread exiting.");
  });
  // -------------------------
  ...
  // ---- FFMPEG PIPELINE CLEANUP ----
  ffw_running = false;
  ffw_cv.notify_one();
  if (ffw_pipe_thread.joinable()) ffw_pipe_thread.join();
  if (ffw_stream_pipe) pclose(ffw_stream_pipe);
  // ---------------------------------
