  ...
  void runGuiLoop()
  {
    ...
    while (!glfwWindowShouldClose(window_) && rclcpp::ok()) {
...

      {
        ...
        // RCLCPP_INFO(get_logger(), "Checking mimics pos %lf %lf joints pair %d ", mj_data_->qpos[mimic_pos_adr_pairs_[i].first], mj_data_->qpos[mimic_pos_adr_pairs_[i].second], i);
        // }
        for ( int i = 0; i < mimic_pos_adr_pairs_.size(); ++i) {
...
private:
    ...
    std::shared_ptr<ffw_collision_checker::srv::CheckCollision::Response> res)
  {
    ...
    if (!actual_collision) {
        ...
        RCLCPP_INFO(get_logger(), "Collision detected | min_dist=%.4f", min_dist);
    }
  }
};
