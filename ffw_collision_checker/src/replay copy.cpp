...
private:
  ...
  void jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    // Update qpos/qvel in mujoco data (only for visualization if no step here)
    for (int i = 0; i < mj_model_->nq; ++i) {
    ...
    // Note: mj_step and publish moved to leaderJointStateCallback
  }
  ...
  void leaderJointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    if (leader_msg_idx_to_actuator_.empty()) {
       leader_msg_idx_to_actuator_.resize(msg->name.size(), -1);
       for (size_t i = 0; i < msg->name.size(); ++i) {
        auto it = joint_name_to_actuator_id_.find(msg->name[i]);
        if (it != joint_name_to_actuator_id_.end()) {
          leader_msg_idx_to_actuator_[i] = it->second;
        }
      }
    }
    ...
    // 2. Override State from latest robot state
    for (int i = 0; i < mj_model_->nq; ++i) {
      mj_data_->qpos[i] = latest_qpos_[i];
    }
    for (int i = 0; i < mj_model_->nv; ++i) {
