...
#
# Debug variant of SMTM for Gazebo simulation with right gripper removed:
#   - Left arm: RH-P12-RN-A gripper (ID 38)
#   - Right arm: NO gripper (7 arm joints only)
#   - Total: 21 joints
#
...
def generate_launch_description():
    declared_arguments = [
        DeclareLaunchArgument('model', default_value='ffw_sg2_smtm_debug',
    ...
    gazebo = IncludeLaunchDescription(
                ...
                launch_arguments=[
                    ('gz_args', [
                        ...
                        ' -r',
                        ' --gui-config ',
                        PathJoinSubstitution([ffw_bringup_path, 'config', model, 'gui.config']),
                    ])
                ]
             )
    ...
    # Spawn robot with gz_ros2_control integration in Gazebo
    ...
    # Initial position executor - sends home trajectory after controllers spawn
    trajectory_params_file = PathJoinSubstitution([
        FindPackageShare('ffw_bringup'),
        'config',
        model,
        'ffw_sg2_smtm_debug_initial_positions.yaml',
    ])
        ...
        *declared_arguments,
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[joint_trajectory_executor_right],
            )
        ),
        bridge,
