...
#
# Fast slider control for simulation
...
class SliderSimControl(Node):
        ...
        for ctrl_key, ctrl in self.controllers.items():
            for i, joint in enumerate(ctrl['joints']):
                if joint in msg.name:
                    ...
                    # Only update value label, never touch slider position
                    # (slider position = user's target, must not be overwritten by feedback)

    def on_slider_drag(self, ctrl_key, joint_idx, value):
        """Called while dragging slider"""
        # Ignore programmatic slider.set updates
    ...
    def check_collision(self, joint_names, joint_positions):
        ...
        try:
            ...
            rclpy.spin_until_future_complete(self, future, timeout_sec=0.015)
            if future.done():
        ...
        for ctrl_key, ctrl in self.controllers.items():
            ...
            if needs_update:
                ...
                if 'prev_velocity' not in ctrl:
                    ...
                    # Initialize commanded_pos from current feedback to avoid huge jumps
                    for i in range(len(ctrl['joints'])):
                ...
                for i in range(len(ctrl['joints'])):
                    ...
                    # print("error %f %f %i" % (error, error * self.kp, i))
                    d_error = (error - ctrl['prev_error'][i]) / self.dt
                    # print("d_error %f %f %i" % (d_error, d_error * self.kd, i))
                    ...
                    ctrl['commanded_pos'][i] = p_cmd  # Store what we commanded
                    ctrl['prev_velocity'][i] = v_cmd

                # Check collision before sending
                if self.collision_client.service_is_ready():
                    if self.check_collision(ctrl['joints'], positions):
                        self.get_logger().warn(f'Collision detected for {ctrl_key}! Command blocked.')
                        # Reset targets to current to prevent further attempts
                        for i in range(len(ctrl['joints'])):
                            ctrl['target'][i] = ctrl['current'][i]
                        continue
                ...
                point.time_from_start.nanosec = 0  # 20ms
